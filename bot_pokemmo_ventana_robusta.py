...
# Tras atacar, el boton LUCHA desaparece durante la animacion.
# No debemos confundir ese estado con el mundo.
POST_ATTACK_GRACE = 2.20
NEXT_BATTLE_MENU_TIMEOUT = 7.0
WORLD_STABLE_TIME = 1.20
...
def wait_after_attack_state(
    hwnd: int,
) -> str:
    """
    Devuelve:
      "BATTLE" -> el menu LUCHA reaparecio; el combate continua.
      "WORLD"  -> no reaparecio LUCHA y el estado sin combate fue estable.
      "EXIT"   -> se pulso F9.
    """

    print(
        f"[STATE] Esperando animacion ({POST_ATTACK_GRACE:.2f}s)..."
    )

    grace_deadline = time.time() + POST_ATTACK_GRACE
    ...
    print("[STATE] Buscando siguiente estado...")

    deadline = time.time() + NEXT_BATTLE_MENU_TIMEOUT
    no_battle_since = None
    ...
    while time.time() < deadline:
        ...
        (
            ...
            size,
            scale,
        ) = detect_battle_menu(hwnd)

        if battle:
            print(
                "[STATE] El menu LUCHA reaparecio: "
                f"el combate continua (score={score:.3f})"
            )
            return "BATTLE"

        if no_battle_since is None:
            no_battle_since = time.time()

        stable_for = time.time() - no_battle_since

        if stable_for >= WORLD_STABLE_TIME:
            print(
                "[STATE] Sin menu de combate estable "
                f"durante {stable_for:.2f}s -> WORLD"
            )
            return "WORLD"

        time.sleep(WORLD_CHECK_INTERVAL)

    print(
        "[STATE] Timeout sin ver LUCHA; "
        "se tratara como WORLD."
    )
    return "WORLD"
...
def attack(
    ...
    print(
        "[2] ESPERANDO MENU DE MOVIMIENTOS "
        f"({PP_CHECK_WAIT:.2f}s)..."
    )
    ...
    print(
        "[6] RESOLVIENDO ESTADO TRAS EL ATAQUE..."
    )

    next_state = wait_after_attack_state(
        hwnd
    )
    ...
    if next_state == "BATTLE":
        print(
            "[7] COMBATE CONTINUA; "
            "se volvera a atacar en el siguiente ciclo"
        )
    elif next_state == "WORLD":
        print(
            "[7] COMBATE TERMINADO; "
